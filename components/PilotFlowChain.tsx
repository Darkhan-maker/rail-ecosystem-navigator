        ...
        {steps.map((step, i) => (
          <div key={step.number} className="flex items-start shrink-0">
            <div className="flex flex-col items-center w-28">
              <div className="w-9 h-9 rounded-full bg-blue-600 text-white flex items-center justify-center text-sm font-bold shrink-0 mb-2">
                {step.number}
              </div>
              <div className="text-xs font-semibold text-gray-800 text-center leading-tight mb-1">{step.name}</div>
              <div className="text-xs text-gray-500 text-center leading-snug">{step.desc}</div>
            </div>
            {i < steps.length - 1 && (
